...
def step_generate(
    ...
    # 排除已入选深挖的条目，避免在分类节中重复出现
    if ddl:
        anchor_keys = set()
        for item in ddl:
            key = item.get("anchor_product") or item.get("title", "")
            if key:
                anchor_keys.add(key)
        filtered_items = [
            item for item in processed_items
            if (item.get("anchor_product") or item.get("title", "")) not in anchor_keys
        ]
        print(f"  过滤重复后: {len(filtered_items)} 条（排除 {len(processed_items) - len(filtered_items)} 条深挖条目）")
    else:
        filtered_items = processed_items

    print(f"  Generating briefing from {len(filtered_items)} items (深挖 {len(ddl)} 条)…")
    briefing = call_llm(
        system_prompt,
        GENERATION_USER.format(
            ...
            items_json=json.dumps(filtered_items, ensure_ascii=False, indent=2),
        ),
        max_tokens=8192,
    )
