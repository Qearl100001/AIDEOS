...
- `data/output/{DATE}-dialogue-doubao.mp3`（豆包语音）或 `{DATE}-dialogue-*.mp3`（`tts_dialogue` preset，以 `specs/dialogue-briefing-agent/output.md` 为准）
...
| Skill | 用途 |
|-------|------|
| **article-dialogue-tts** | 豆包整篇口播：`python -m tools.article_tts`；备选 `tools.tts_dialogue`（Edge） |
...
│  article.md → *.mp3（主：豆包 article_tts；备选 tts_dialogue）│
└─────────────────────────────────────────────────────────────┘
```
...
- **主入口（豆包语音合成大模型 V3）**：`python -m tools.article_tts [--date YYYY-MM-DD]`；鉴权 **`DOUBAO_API_KEY`（推荐）** 或 **`DOUBAO_APP_ID` + `DOUBAO_ACCESS_TOKEN`**；必填 **`DOUBAO_RESOURCE_ID`**（如 `seed-tts-2.0`）；可选 **`DOUBAO_VOICE_Q` / `DOUBAO_VOICE_A`**（`speaker`，Q 男、A 女）。默认读 **`data/output/{DATE}-article.md`**，输出 **`{DATE}-dialogue-doubao.mp3`**。文档总览：<https://www.volcengine.com/docs/6561/1329505?lang=zh>；详见 **`.claude/skills/article-dialogue-tts/`**。  
- **备选**：`python -m tools.tts_dialogue [--preset stereo|edge|…]`；`--list-presets` 查看预设（Edge 等）。  
