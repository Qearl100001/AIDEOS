...
def _pick_voice(preset: DialoguePreset, speaker: str) -> str:
    """Q / Qearl → qearl_voice（男），A / N → lidada_voice（女）。"""
    u = speaker.strip().upper()
    if u == "Q" or u == "QEARL" or "QEARL" in u:
        return preset.qearl_voice
    # A (answer) and N (narrative) both use female voice
    if u == "A" or u == "N":
...
def run_preset(preset_id: str, date_str: str | None = None) -> Path | None:
    ...
    if preset.include_narrative:
        matches = parse_all_segments(text)
        print(f"[tts] 解析到 {len(matches)} 段（包括对话+叙述）")
    else:
