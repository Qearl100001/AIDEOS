...
| 4 语音 | `BRIEFING_DATE=… python3 -m tools.article_tts`（豆包）或 `tts_dialogue --preset edge` 等 | `{DATE}-dialogue-doubao.mp3` 或 `{DATE}-dialogue-*.mp3` |
...
```bash
# 主路径：豆包 V3（需 DOUBAO_API_KEY 或 DOUBAO_APP_ID+DOUBAO_ACCESS_TOKEN，及 DOUBAO_RESOURCE_ID，见 skill）
