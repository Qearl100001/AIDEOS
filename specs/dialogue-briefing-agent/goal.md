...
1. **音频文件（主交付）**：`data/output/YYYY-MM-DD-dialogue-doubao.mp3`（豆包，`python -m tools.article_tts`）或 `dialogue-*.mp3`（`tts_dialogue` 备选）  
   - **唯一朗读母本**：`data/output/YYYY-MM-DD-article.md`（由 Briefing Article Agent 产出的对话式结构化文章）  
   - 使用 TTS（主：豆包语音；备选：Edge 等，见 `article-dialogue-tts` skill 与 `tools/tts_dialogue`）
...
2. **角色配音**：`article.md` 中 **`Q` / `A`**（或历史 **Qearl**）：**Q 男声、A 女声**，与 `article-dialogue-tts` / `DOUBAO_VOICE_*` 或 `tts_dialogue` preset 一致。  
